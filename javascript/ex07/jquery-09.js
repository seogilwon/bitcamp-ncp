...
//7.
el.appendTo = function(parents) {  //자식태그를 복제해서 각 부모태그에 붙인다.
  for (let parent of parents) {
    for (let child of el) {     //자식들이 들어있는 상자에서 자식을 한개씩꺼내 복제하여 각 부모의 자식으로붙인다.
    parent.appendChild(child.cloneNode(true));
  }
}

for (let child of el) {      //자식태그는 제거한다.
  if (child.parentElement != null || child.parentElement != undefined) {
   child.parentElement.removeChild(child);
 }
}
...
el.click = function(handler) {          //handler=listener=callback
  for (let e of el) {
